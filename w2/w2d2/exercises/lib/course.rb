...
class Course
  ...
  def initialize(name, department, credits, days = [], block = 1)
    @name = name
  ...
  def conflicts_with?(other_course)
    other_days = other_course.days
    other_time = other_course.time_block

    @days.each do |day|
      if other_days.include?(day) && @time_block == other_time
        return true
        break
      end
    end
